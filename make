...
kernel_library="https://github.com/lynxnexy/kernel/tree/main/pub"
#kernel_library="https://github.com/kyliezero/kernel/tree/main/pub"
...
EOF
    ...
    [ -f etc/modules.d/8189fu ] || echo "8189fu" >etc/modules.d/8189fu
    ...
    #    cp -f ${gpio_file}/gpionet usr/bin/gpionet && chmod +x usr/bin/gpionet >/dev/null 2>&1
    #    cp -f ${gpio_file}/gpiotemp usr/bin/gpiotemp && chmod +x usr/bin/gpiotemp >/dev/null 2>&1
    #    sed -i "/exit/i\sleep 20 && /usr/bin/gpionet -r" etc/rc.local >/dev/null 2>&1
